...
This file is the single canonical instruction document for Codex,
Codex, and other coding agents working in this repository. Shared rules,
skills, agent playbooks, and knowledge remain under `.Codex/`; do not create
...
`.Codex/`. In particular, this project's small, single-player jam scope and
...
- Codex uses the existing `.Codex` settings, agent, skills, and
...
- Codex uses `.codex/config.toml` to discover this file and
  ...
  Markdown under `.Codex/`.
- Codex-only frontmatter, tool names, model settings, hooks, memory behavior,
...
- Root `AGENTS.md` and `.agents/` are intentionally absent. Do not recreate
  them or translate `.Codex` content into a Codex-specific copy.

### Agent routing

| Agent | Canonical playbook | Use for |
|---|---|---|
| `godot-advisor` | `.Codex/agents/godot-advisor.md` | Non-trivial gameplay design, Godot/C# architecture, code review, pattern selection, and implementation or refactoring advice |
...
2. Read `.Codex/agents/godot-advisor.md` when the specialist domain applies.
3. Read the matching `.Codex/skills/<name>/SKILL.md` and only the rules or
...
Codex may expose these as slash commands. Other runtimes should treat
the names as shared workflows and load the same `.Codex/skills/` source.
...
- Resolve conflicts in this order: correctness, simplicity, documented
  project intent, readability, then measured performance. `AGENTS.md` is the
...
- Godot **4.7** (`config/features` in `project.godot`), pinned at 4.7.1 per
  `.Codex/knowledge/decisions/004-godot-version-pin-4.7.1.md`.
...
- `TreatWarningsAsErrors`, `AnalysisLevel=latest`, and
  ...
  `.Codex/knowledge/decisions/006-nasa-power-of-ten-adapted-for-godot-csharp.md`.
...
`.Codex/knowledge/decisions/007-unit-test-harness-scoped-to-pure-logic.md`
...
- **Sim-view split, project-wide.** Gameplay rules live in
  ...
  catch. See `.Codex/knowledge/decisions/009-sim-view-separation.md`.
...
- [`.Codex/rules/doctrine.md`](.Codex/rules/doctrine.md) — the
...
- [`.Codex/rules/priorities.md`](.Codex/rules/priorities.md) — the
...
- [`.Codex/rules/godot-csharp-conventions.md`](.Codex/rules/godot-csharp-conventions.md) —
...
- [`.Codex/rules/performance.md`](.Codex/rules/performance.md) — free
  habits vs. complexity-adding tools, plus the Godot-specific hot-path
  list.
- [`.Codex/rules/skill-authoring.md`](.Codex/rules/skill-authoring.md) —
  conventions for this project's own `.Codex/skills/*` files.
- `.Codex/knowledge/decisions/` — nine ADRs recording calls already made
...
- [`.Codex/knowledge/godot-csharp-gotchas.md`](.Codex/knowledge/godot-csharp-gotchas.md),
  [`multithreading-csharp-godot.md`](.Codex/knowledge/multithreading-csharp-godot.md),
  [`gaming-patterns-index.md`](.Codex/knowledge/gaming-patterns-index.md) —
